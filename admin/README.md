...
已完成第一批候选审核后台骨架，并接入真实 `/api/admin/candidates` 列表、详情、发布和拒绝接口；已完成简报管理页，支持 mock/API 双模式创建每日简报草稿、编辑草稿、发布到 APP 和下线已发布简报。下一阶段建议补 Flutter API 模式回归、OpenAPI 示例、操作日志和采集任务管理页。
