...
当前阶段已完成独立工程骨架和候选资讯审核页面，不是旧的用户端 React/Vite PWA。页面支持未配置后端时使用 mock 数据，也支持配置 Spring Boot Admin API 后进入真实联调模式。
...
已完成第一批候选审核后台骨架，并接入真实 `/api/admin/candidates` 列表、详情、发布和拒绝接口。下一阶段建议补充候选编辑表单、批量审核、操作日志和采集任务管理页。
