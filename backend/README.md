...
DeepSeek 还可通过 `PULSEBRIEF_DEEPSEEK_CLASSIFICATION_ENABLED=true` 独立开启候选模型分类。关键词规则保持优先；模型只接收标题、来源和 RSS 摘要，低置信度或失败时回退来源默认分类，最终分类仍由 Admin 确认。详见 `docs/DeepSeek模型分类接入说明.md`。

真实 DeepSeek 摘要与分类请求会写入统一 AI 用量事件，并受每日请求数和 Token 预算保护。Admin 可查看今日请求、Token、估算成本与限流告警；供应商单价通过环境变量配置，不在代码中硬编码。详见 `docs/AI用量成本限流告警说明.md`。
