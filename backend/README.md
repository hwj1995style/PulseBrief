...
最新内容与授权全文/PDF 采集设计见 `docs/最新内容与授权全文PDF采集V1设计.md`。真实 Provider 必须默认只采集最新内容：新闻默认 24 小时窗口，公开报告和授权 PDF 默认 72 小时窗口；付费研报、登录后 PDF、历史研报库和未授权全文不进入 V1。当前后端已支持采集源许可字段和最新窗口过滤；授权全文字段、`report_asset` 与 PDF 文件缓存后续分阶段实现。
