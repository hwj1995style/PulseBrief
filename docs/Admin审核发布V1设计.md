...
4. 本阶段不实现自动发布。
5. 本阶段不实现真实 AI 摘要服务。
6. 本阶段不实现付费研报、登录后 PDF 或未授权全文采集；授权公开 PDF 和授权全文按 `docs/最新内容与授权全文PDF采集V1设计.md` 分阶段实现。
...
V1 默认不写新闻全文，不生成真实音频文件，不下载远端图片。对于授权明确且属于最新窗口的来源，可在后续阶段写入授权全文或授权 PDF 元数据，并且必须经过 Admin 合规审核。
...
3. 原文链接。
4. 可能重复信息，V1 可先返回空数组。
5. 内容许可策略、正文片段预览和 PDF 元数据，若存在。
6. 当前可执行动作：`EDIT`、`REJECT`、`PUBLISH`、`APPROVE_COMPLIANCE`。
...
5. 若候选包含授权全文或 PDF，发布前必须通过合规审核；未通过时返回 `409 CONFLICT`。
