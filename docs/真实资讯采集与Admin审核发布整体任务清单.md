...
- [x] 补充 `docs/真实资讯数据采集V1设计.md`。
- [x] 明确 V1 数据源范围：全球热点、财经市场、科技趋势、AI 动态、投行公开观点。
- [x] 明确禁止范围：付费研报、登录后 PDF、未授权全文转载、绕过反爬、未授权图片采集。
- [x] 设计原始采集表、候选资讯表、采集任务日志表。
- [x] 设计去重规则：`source + originalUrl`、标题指纹、发布时间窗口。
- [x] 设计分类规则：关键词规则 + 来源默认分类 + Admin 可改。
- [x] 设计采集状态流转：`NEW / DUPLICATE / CANDIDATE / REJECTED / PUBLISHED / ERROR`。
- [x] 设计失败重试、限流、手动触发采集和任务日志查看。
...
阶段结果：已完成运营质量与监控 V1 设计，并落地 Admin 监控能力：采集任务列表、失败错误信息、今日采集/候选/发布/失败统计、采集源许可状态展示、采集源启用/停用配置、文章发布/简报发布/简报下线操作日志，以及原始资讯缺来源、缺原文链接、缺发布时间和未来发布时间异常检测。
