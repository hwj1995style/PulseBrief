...
3. 支持查看简报列表、选择已发布文章、创建草稿、编辑草稿、发布草稿、下线已发布简报。
...
3. 左侧主区域：简报列表、草稿表单、文章候选池。
4. 右侧详情栏：当前草稿/已发布简报详情、热点清单、发布按钮、下线按钮。
...
4. 点击草稿的“编辑草稿”后，将标题、摘要、播报文案和文章热点加载到表单。
5. 保存草稿时调用 `PUT /api/admin/digests/{id}`，只允许编辑 `DRAFT`。
6. 发布草稿后更新本地简报列表状态。
7. 下线已发布简报时调用 `POST /api/admin/digests/{id}/offline`，状态变为 `OFFLINE`。
8. API 模式失败时展示错误，不阻断 mock 模式本地预览。
...
2. `adminApi.test.ts`：覆盖编辑草稿和下线简报的 HTTP 映射。
3. `App.test.tsx`：覆盖进入简报管理页、选择文章、创建草稿、编辑草稿、发布草稿、下线已发布简报。
...
1. 文章候选池当前后端按 `findAll()` 过滤，数据量增大后需要分页和搜索优化。
2. Mock 与 API 响应结构若后续分化，可能导致页面状态不一致。
3. 已发布简报下线会影响用户端可见性，操作前需要明显文案提示。

分阶段建议：

1. 第一批实现创建和发布闭环。
2. 第二批补后端编辑/下线接口，并在 React Admin 增加编辑态和下线操作。
