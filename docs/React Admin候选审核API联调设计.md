...
当前阻塞点是浏览器端尚未真正调用后端接口：前端缺少 HTTP API client、字段映射、错误状态和真实接口模式配置；后端也需要允许 Vite 本地开发端口对 `/api/admin/**` 发起跨域预检请求。
...
- 增加 API client 测试，覆盖列表映射、详情映射和发布/拒绝调用。
...
3. 列表页启动时加载候选池，筛选在前端内存完成；刷新按钮重新拉取。
4. 点击候选行时优先加载详情，补齐 PDF 资产与抓取时间。
5. 发布/拒绝成功后更新本地候选状态并保留选中项。
6. 出错时展示可读错误提示，不阻塞 mock 模式使用。
